...
class ProjectAttachmentCustomForm extends BaseForm
{

  private $projectAttachmentCollection = array();

  public function getProjectAttachmentCollection(){
    return $this->projectAttachmentCollection;
  }
  ...
  public function bindMultipleFiles($files = array()) {
    foreach($files as $index => $file) {
	    ...
	    if (move_uploaded_file($_FILES[$index]['tmp_name']['file'], sfConfig::get('sf_upload_dir').'/'.$path)) {
               ...
               $projectAttachment->setValue($path);
               $this->projectAttachmentCollection[] = $projectAttachment;
