...
class StuffAttachmentCustomForm extends BaseForm
{

  private $stuffAttachmentCollection = array();
 
  public function getStuffAttachmentCollection(){
    return $this->stuffAttachmentCollection;
  }
  ...
  public function bindMultipleFiles($files = array()) {
    foreach($files as $index => $file) {
	    ...
	    if (move_uploaded_file($_FILES[$index]['tmp_name']['file'], sfConfig::get('sf_upload_dir').'/'.$path)) {
               ...
               $stuffAttachment->setValue($path);               
               $this->stuffAttachmentCollection[] = $stuffAttachment;
